subroutine inertmv2_ref(n, m1, m2, mass, x1, x, v1, v2)
	use kinds, only: DP
	implicit none
	!---------------------------------------------------------------------------
	! Input Variables
	...
	! Output Variables
	!
	real(DP), intent(inout) :: x(3,n), v1(3,m1), v2(3,m2), x1(3,n)
	!---------------------------------------------------------------------------
	! Local Variables
	!
	integer               :: iat, ix, ier
	...
	do itype=1, 4
		...
		call rotn(m1, vector1, u1)
		call rotn(m2, vector1, u2)
		!-------------------------------------------------------------------------
		test_dx=0.D0
		do i=1, n
			do j=1, 3
	...
	call rotn(m1, vector, v1)
	call rotn(m2, vector, v2)
	!----------------------------------------------------------------------------
	deallocate(x2)
	deallocate(u1)
	deallocate(u2)
