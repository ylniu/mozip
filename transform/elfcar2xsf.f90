program chg2xsf
	...
	character(200)               :: line, tmp, finp, middlez
	...
	!----------------------------------------------------------------------------
	open(fid, file="ELFCAR", status="old")
		read(fid,*) line
		read(fid,*) alat
		read(fid,*) a(:,1)
		read(fid,*) a(:,2)
...
! 		tot_dens = tot_dens / vol
		...
		if (trim(middlez)=="middlez") then
			...
			do iz=2, nz
				dtot_dens(:,:,iz) = tot_dens(:,:,iz) - tot_dens(:,:,iz)
			end do
		end if
		call write_xsf_3dgrid("ELFCAR.xsf", a, natom, symbol, x, nx, ny, nz,  tot_dens)
		deallocate(tot_dens)
...
! 			spin_dens = spin_dens / vol
			...
			call write_xsf_3dgrid("ELFCAR_spin.xsf", a, natom, symbol, x, nx, ny, nz,  spin_dens)
